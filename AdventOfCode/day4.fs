...
let countValid input =
    let split (separators:char []) (s:string) = s.Split(separators, StringSplitOptions.RemoveEmptyEntries)
    let isValid  (s:string) =
        let splitted = s |> split [|' '|] |> Array.map (fun x -> x |> Seq.sort |> String.Concat)
        let count = splitted.Length
        let set = splitted |> Set.ofArray
        count = set.Count

    input 
    |> split [|'\n';'\r'|]
    |> Seq.sumBy (fun x -> if isValid x then 1 else 0)


countValid testInput |> ignore
countValid actualInput |> ignore
