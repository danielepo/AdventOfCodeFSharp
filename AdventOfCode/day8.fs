...
let parseInput (input:string list)=
    let getRegister (registers:Map<'a, int>) name =
        match registers.TryFind name with
        | Some v -> v
        | None -> 0


    let executeInstruction (registers:Map<string, int>) (inst:Instruction) =
        let (c, i) = inst.Comparer, inst.Instruction 
        //| Some c, Some i -> 
        let comValue = (getRegister registers inst.ComReg)
        let value = (getRegister registers inst.RegName)

        if c comValue inst.CompVal 
        then  registers.Add (inst.RegName, i value inst.Value)
        else registers

        //| _, _ -> registers
    
         
    let toRegInstr (input:string[])=
         {
            RegName = input.[0] //c
            Instruction = toInstruction input.[1] // dec 
            Value = input.[2] |> int // 10
            ComReg= input.[4] //a
            Comparer= toComparer input.[5]  // >
            CompVal = input.[6] |> int // 1
            }
